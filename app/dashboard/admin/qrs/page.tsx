...
export default async function AdminQRsPage({ searchParams }: Props) {
  ...
  const title =
    filter === "active" ? "QR Activos" : filter === "used" ? "QR Usados" : "Todos los QR";
  ...
  return (
    <main className="min-h-screen p-4 max-w-md mx-auto">
      ...
      <h1 className="text-2xl font-bold mt-4 mb-2 text-slate-800">{title}</h1>
      <p className="text-sm text-slate-500 mb-6">{qrs.length} QR encontrados</p>
